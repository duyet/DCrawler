...
;(function () {
  ...
  Instance.prototype.validURL = function (str) {
  ...
  Instance.prototype.start = function () {
    ...
    this.instanceConfig.callback = function (error, result, $) {
      ...
      // ====================================================
      // Paging
      var pagingWraps = $(that.getPagingSelector())

      if (pagingWraps && pagingWraps.length > 0) {
        for (var i = 0; i < pagingWraps.length; i++) {
          var pagingWrap = pagingWraps[i] || {};
          ...
          if (paging.length) {
            paging.each(function (index, a) {
              ...
              if (a != null && (nextPage = $(a).attr('href')) != null) {
                ...
                if (!that.validURL(nextPage)) nextPage = that.instanceConfig.baseUrl + '/' + nextPage
      ...
      if (container.length) {
        container.each(function (index, block) {
          ...
          var collectData = {
            ...
            content: that.getContentBySelector($(block), that.getElRules('content'), 'None'),
            price: that.getContentBySelector($(block), that.getElRules('price'), '0'),
            title: that.getContentBySelector($(block), that.getElRules('title'), 'None'),
            label: '', // default empty
          }
